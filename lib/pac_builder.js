...
export function formatProfileToPacString(profile) {
  ...
  if (profile.type === 'fixed') {
    ...
    switch ((scheme || 'http').toLowerCase()) {
      ...
      case 'https':
        // 注意：HTTPS 节点是 TLS 端口，绝不能附带明文 PROXY fallback（明文 CONNECT
        // 会被 TLS 服务器丢弃导致连接失败）。Chrome/Firefox 均支持 PAC 的 HTTPS 关键字。
        return `HTTPS ${cleanHost}:${cleanPort}; DIRECT`;
      case 'http':
      default:
...
function FindProxyForURL(url, host) {
  ...
  if (
    url.indexOf('__direct_test=1') !== -1 ||
    shExpMatch(host, 'connectivitycheck.gstatic.com') ||
    shExpMatch(host, 'cp.cloudflare.com')
  ) {
    return "DIRECT";
  }
